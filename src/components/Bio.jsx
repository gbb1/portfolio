...
const Bio = ({ scroll }) => {
  ...
  const observer = useObserver(mainRef, { freezeOnceVisible: true, threshold: 0.0 });
  const title = useTyper("Hi! I'm Gabe.", observer?.isIntersecting)
  const sub = useTyper("I'm a fullstack engineer with a background in Stats and Linguistics, and professional experience in UX design and marketing.", observer?.isIntersecting)

  // const percentage = scroll >= 0 ? Math.min(100, viewportHeight * 2 / scroll) : 100
  const percentage = 100

  return (
   <div ref={mainRef} className={`absolute w-full h-full z-[9] flex flex-col items-start ease-in-out opacity-[${percentage}%]`} style={{ translate: `0px ${scroll}px`}}>
